...
//        $('#FromDate').val(start.format('YYYY-M-DD'));
//        $('#ToDate').val(end.format('YYYY-M-DD'));
//        GetData(start.format('YYYY-M-DD'), end.format('YYYY-M-DD'));
//    }
...
$(document).ready(function () {
    ...
    Getcalltypedatainarray(2020)

    GetINTCNTinarray(2020, displayALLcalltype)
    gettransactiontype();
    // ddbranch();
    yeardropdown();
    getcountryingrid(2020);
    ...
    $("#ddtransaction").jqxDropDownList(
        {
            ...
            // selectedIndex: ,
            displayMember: 'TRANS_TYPE',
            valueMember: 'TRANS_TYPE'
        });
    // $("#ddtransaction").val("Others");
    GetData();
    // subscribe to the select event.
    $("#ddtransaction").on('select', function (event) {
        if (event.args) {
            debugger;
...
function yeardropdown() {
    ...
    $("#jqxyeardd").on('select', function (event) {
    ...
    $("#jqxyeartransactiondd").on('select', function (event) {
        if (event.args) {
            debugger;
            var item = event.args.item;
    ...
    $("#jqxyearcalltypendd").on('select', function (event) {
        if (event.args) {
            debugger;
            var item = event.args.item;
            if (item) {
    ...
    $("#jqxyearINTcountdd").on('select', function (event) {
        if (event.args) {
            debugger;
            var item = event.args.item;
            if (item) {
    ...
    $("#ddtransactionforINTCNTRY").on('select', function (event) {
        if (event.args) {
            debugger;
            var item = event.args.item;
            if (item) {
                ...
                var valueelement = $("<div></div>");
                valueelement.text("Value: " + item.value);
                var selectedcalltype = item.value
    ...
    $("#jqxyearselectioningriddd").on('select', function (event) {
        if (event.args) {
            debugger;
            var item = event.args.item;
            if (item) {
    ...
    $("#jqxmonthselectioningriddd").on('select', function (event) {
        if (event.args) {
            debugger;
            var item = event.args.item;
            if (item) {
                ...
                if ($("#jqxyearselectioningriddd").val() == '') {
                    selectedmonthforgrid = item.value
                    alert("year not selected")
                    return;
                }
