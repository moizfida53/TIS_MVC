...
$(document).ready(function () {
    ...
    var dropDownContentCompany = '<div style="position: relative; margin: 3px; ">Select Company</div>';
    $("#btnCompany").jqxDropDownButton({ width: 250, height: 25 });
    $("#btnCompany").jqxDropDownButton('setContent', dropDownContentCompany);
    $('#btnCompany').on('open', function () { FillCompany(); });
...
function FillGrid() {
    ...
    $("#grdEmployee").on('rowselect', function (event) {
        ...
        $("#btnCompany").jqxDropDownButton('setContent', COMPANY);
    });
}
...
function AddEmployee() {
    ...
    if (selectedCompany == 0) {
        ...
        $("#btnCompany").notify('Please Select Company', { position: "right" });
        return;
    }
...
function Clear() {
    ...
    $('#btnAdd').show();
    /*selectedCompany = 0;*/
    $("#btnCompany").jqxDropDownButton('setContent', '');
    $("#btnCompany").jqxDropDownButton('setContent', '<div style="position: relative; margin: 3px;">Select Company</div>');
}
function OpenCC() {
    FillGridCC();
    $('#btnUpdate').hide();
...
function FillCountry() {
    ...
    if (!$("#lbCountry").length) {
        console.error("lbCountry div not found!");
        return;
    }
    ...
    var dataAdapterCnt = new $.jqx.dataAdapter(source, { autoBind: true });
    ...
    console.log("jqxListBox initialized with items:", $("#lbCountry").jqxListBox('getItems').length);
}

function FillCountry() {
    var $lb = $("#lbCountry");
    $lb.empty(); // Clear existing options

    if (!Array.isArray(Countries) || Countries.length === 0) {
        $lb.append('<option disabled>No countries available</option>');
        return;
    }

    // Create a multi-select <select>
    var $select = $('<select>', {
        id: 'selCountry',
        multiple: 'multiple',
...
function FillCompany() {
    ...
    $("#grdCompany").on('rowselect', function (event) {
        debugger;
        var args = event.args;
        var row = $("#grdCompany").jqxGrid('getrowdata', args.rowindex);
        var dropDownct = '<div style="position: relative; margin-left: 3px; margin-top: 5px;">' + row['COMPANY'] + '</div>';
        $('#hidCompany').val(row['ID']);
        selectedCompany = row['ID'];
        $("#btnCompany").jqxDropDownButton('setContent', dropDownct);
        $('#btnCompany').jqxDropDownButton('close');
    });
...
function ExportToExcel() {
    saveMyFile($('#SubmitForm'), "employee_list" + ".xls", $("#grdEmployee").jqxGrid('exportdata', 'xls'));
}
